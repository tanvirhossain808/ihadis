...
const Section = ({ title, numbers, h, w, code, isSelected, getChapter, id, isBookTrue }) => {
    const [isHover, setIsHover] = useState(false)
    return (<>
        <title>{title}</title>
        <div className={`${h} ${w} ${isSelected && "bg-[#EBFCF6]"} items-center p-4 rounded-2xl flex gap-6  cursor-pointer`} onMouseEnter={() => setIsHover(!isHover)} onMouseLeave={() => setIsHover(!isHover)} onClick={() => getChapter(id)}>
            <div>

                <div className=" ">
                    <span className="relative">
                        <svg className={`${isSelected || isHover ? "fill-[#2B9E76]" : "fill-[#f1f5f4]"}`} width="50" height="50" viewBox="0 0 56 62" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <h3 className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-x ${isSelected || isHover ? "text-white" : " text-gray-400 "}`}>
                            {code}
